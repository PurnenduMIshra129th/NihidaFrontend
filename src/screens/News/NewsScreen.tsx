    ...
    return (
        <>
            <div className="flex justify-center items-center flex-col sm:w-[80%] w-full p-[1rem] sm:p-0 m-3">
                <SubHeading text="News" />
                <Heading text="Our recent news of events" className="text-custom_maroon text-center" />
                {
                    apiData && <div className="flex sm:justify-end justify-center w-full my-3">
                        <ViewAllButton text="View All News" onClick={() => navigate("/users/view-all-news")} />
                    </div>
                }

                <div className="flex flex-row justify-center items-center flex-wrap w-full gap-[1.5rem] my-3">
                    {!apiData || apiData.length === 0 ? <NoDataComponent message="No news available at the moment" /> : null}
