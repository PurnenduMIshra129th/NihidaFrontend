        ...
        {data?.focusAreas && data?.focusAreas?.length > 0 && (
          <div className="flex flex-wrap gap-1 text-xs mb-4">
            {data.focusAreas.map((area, idx) => (
              <span
                key={idx}
                className="bg-gray-50 border border-orange-200 text-orange-700 px-2 py-0.5 rounded-full"
              >
                {area}
              </span>
            ))}
